(function () {
  ...
  const groupDefinitions = [
    ...
    { id: "backend", label: "Backend", parent: "development", hidden: true },
    { id: "framework", label: "Framework", parent: "backend", parentCategory: "backend" },
  ...
  function buildGraph() {
    ...
    if (!category) {
      const visibleGroups = groupDefinitions.filter((group) =>
        group.parent === (state.groupId || null) && !group.hidden && !group.parentCategory);
      if (state.groupId) {
    ...
    if (!article) {
      ...
      groupDefinitions.filter((group) => group.parentCategory === category.id).forEach((group) => {
        nodes.push({
          id: "group:" + group.id,
          sourceId: group.id,
          type: "group",
          label: group.label,
          count: categories.filter((item) => item.parentGroup === group.id).length,
          depth: 2
        });
        edges.push({ source: "category:" + category.id, target: "group:" + group.id });
      });
  ...
  async function activate(node) {
    if (node.type === "group") {
      ...
      const returnCategory = state.groupId === node.sourceId && group && group.parentCategory;
      state.groupId = returnCategory
        ? groupDefinition(group.parent)?.parent || null
        : state.groupId === node.sourceId ? (group && group.parent) : node.sourceId;
      state.categoryId = returnCategory ? group.parentCategory : null;
      state.seriesId = null;
      state.articleId = null;
      state.sections = [];
      render();
      updateInspector(returnCategory
        ? { type: "category", sourceId: group.parentCategory }
        : state.groupId ? node : null);
      return;
    }
